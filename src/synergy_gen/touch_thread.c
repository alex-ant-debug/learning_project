...
{ .p_lower_lvl_i2c = &g_touch_i2c, .p_lower_lvl_irq = &g_sf_external_irq, .pin = IOPORT_PORT_06_PIN_09, .p_chip =
          &g_sf_touch_panel_i2c_chip_sx8654, };
const sf_touch_panel_cfg_t g_sf_touch_panel_i2c_cfg =
{ .hsize_pixels = 240, .vsize_pixels = 320, .priority = 5, .update_hz = 10, .p_message = &g_sf_message0, .p_extend =
...
void touch_thread_create(void)
{
    ...
    err = tx_thread_create (&touch_thread, (CHAR *) "Touch Thread", touch_thread_func, (ULONG) NULL,
                            &touch_thread_stack, 1024, 6, 6, 1, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
